...
if __name__ == "__main__":
    ...
    data_loading_epoch = []
    comm_time_epoch = []
    train_dict = {"compute_time": [], "comm_time_epoch" : [], "data_loading_time" : [], "batch_size": [], "gpu_count": []}
    train_accuracies = []
    train_losses = []
    ...
    for device_count in [1,2,4]:
        for batch_size in [32,128,512]:
            ...
            train_dict["compute_time"].append(compute_time_epoch[-1])
            train_dict['data_loading_time'].append(data_loading_epoch[-1])
            train_dict["comm_time_epoch"].append(compute_time_epoch[-1] - data_loading_epoch[-1] - comm_time[batch_size])
    
    pd.DataFrame.from_dict(train_dict).to_csv("output6-2.2.csv")
